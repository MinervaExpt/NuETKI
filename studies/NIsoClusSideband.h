...
class NIsoClusSideband: public Study
{
  private:
  ...
  //what args do I need for my sideband constructor??
    NIsoClusSideband(std::vector<Variable*> vars,
      ...
      for (auto& var : vars){
	fVars.push_back(new Variable((var->GetName()+"_NIsoClusSB").c_str(), var->GetAxisLabel(), var->GetBinVec(), var->GetRecoFunc(), var->GetTrueFunc()));
      }
  ...
  private:
	...
	//std::cout << "Entering NIsoClusSideband::fillSelectedMC" << std::endl;
	...
	g_OutputTreeManager.Fill("NIsoClus_Sideband", evt.entryNumber); //add this event to the michel sb tree of my output tree, selectionCategory is already set earlier
