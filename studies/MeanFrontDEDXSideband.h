...
class MeanFrontDEDXSideband: public Study
{
  ...
  private:
	...
	if (univ.GetTruthNuPDG()==12 && univ.GetCurrent()==1 && univ.GetElectronEnergyTrue()>=2.5 && univ.GetHasSignalFSProton()==1 && univ.GetHasFSMeson()==0 && univ.GetHasFSPhoton()==0 && univ.GetTrueVertex().z() >= 5980 && univ.GetTrueVertex().z() <= 8422 && inApothem) {
